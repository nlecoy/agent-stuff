...
export const SENTRY_API_BASE = getSentryApiBase();

function getSentryApiBase() {
  // 1. Env var override
  if (process.env.SENTRY_BASE_URL) {
    return process.env.SENTRY_BASE_URL.replace(/\/$/, "") + "/api/0";
  }
  // 2. Read from ~/.sentryclirc [defaults] url
  try {
    const rcPath = join(homedir(), ".sentryclirc");
    if (existsSync(rcPath)) {
      const content = readFileSync(rcPath, "utf-8");
      const match = content.match(/\[defaults\][^\[]*url\s*=\s*(.+)/s);
      if (match) {
        return match[1].trim().replace(/\/$/, "") + "/api/0";
      }
    }
  } catch {
